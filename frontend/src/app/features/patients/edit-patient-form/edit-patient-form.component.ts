...
// A date string from the API (e.g. "2022-03-15T00:00:00Z") needs trimming to
// yyyy-MM-dd for an <input type="date"> to display it as pre-filled.
...
@Component({
  ...
  styleUrl: './edit-patient-form.component.scss'
})
export class EditPatientFormComponent implements OnInit {
  ...
  readonly updating$: Observable<boolean> = this.store.select(selectPatientUpdating);
  ...
  constructor(
    ...
    this.actions$.pipe(ofType(PatientsActions.updatePatientSuccess), takeUntilDestroyed()).subscribe(() => {
    ...
    this.actions$
      .pipe(ofType(PatientsActions.updatePatientFailure), takeUntilDestroyed())
      .subscribe(({ error }) => {
  ...
  ngOnInit(): void {
    this.form.setValue({
      ...
      dateOfBirth: toDateInputValue(this.patient.dateOfBirth),
      phoneNumber: this.patient.phoneNumber ?? '',
      email: this.patient.email ?? '',
      deviceType: this.patient.deviceType,
      deviceManufacturer: this.patient.deviceManufacturer ?? '',
      deviceModel: this.patient.deviceModel ?? '',
      deviceSerialNumber: this.patient.deviceSerialNumber,
      implantDate: toDateInputValue(this.patient.implantDate)
    });
  }
